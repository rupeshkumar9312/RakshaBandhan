import { cn, formatDate, STATUS_LABELS, type OrderStatus } from "@/lib/utils";
import { formatPaise } from "@/lib/money";
import { CheckIcon, PackageIcon } from "@/components/icons";

const TIMELINE: Exclude<OrderStatus, "CANCELLED">[] = [
  "PENDING",
  "PROCESSING",
  "SHIPPED",
  "DELIVERED",
...
const TIMELINE_COPY: Record<Exclude<OrderStatus, "CANCELLED">, string> = {
  PENDING: "We've received your order.",
  PROCESSING: "Your rakhis are being packed.",
  SHIPPED: "On the way to your tower.",
  DELIVERED: "Handed over. Happy Raksha Bandhan!",
};
...
/** The step-by-step status card shown on the public order page and the /track lookup. */
export function OrderTracker({ order }: { order: TrackedOrder }) {
  const currentIndex = TIMELINE.indexOf(order.status as Exclude<OrderStatus, "CANCELLED">);
    ...
    <div className="card p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-ink-muted">Placed {formatDate(order.placedAt, true)}</p>
        </div>
        <span
          className={cn(
            "chip",
            ...
            return (
              <li key={status} className="flex gap-3.5">
                <div className="flex flex-col items-center">
                  ...
                  >
                    {done ? <CheckIcon className="size-4" /> : <PackageIcon className="size-4" />}
                  </span>
                  {i < TIMELINE.length - 1 && (
                    <span
                      className={cn(
                ...
                <div className={cn("pb-6", i === TIMELINE.length - 1 && "pb-0")}>
                  ...
                  <p className="mt-0.5 text-xs text-ink-muted">{TIMELINE_COPY[status]}</p>
                </div>
              </li>
            );
          })}
      ...
      <div className="mt-4 flex justify-between border-t border-cream-300 pt-4">
        <span className="font-bold">
          Total {order.status === "DELIVERED" ? "paid" : "payable"}
        </span>
        <span className="font-bold tabular-nums text-maroon-800">{formatPaise(order.total)}</span>
      </div>
    </div>
  );
}
